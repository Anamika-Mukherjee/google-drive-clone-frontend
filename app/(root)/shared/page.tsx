...
const Page = () =>{
    ...
    //function to get shared file data 
    useEffect(()=>{
        if(isClient){
            ...
            //call function to get shared files 
            getFiles()
            .then((response)=>{
    ...
    //calculate total size of all shared files when file data received
    useEffect(()=>{
        let totalSize = 0;
    ...
    return(
        <div className="w-full h-full flex flex-col justify-start items-center space-y-3">
            <section className="w-full h-auto flex flex-col justify-start items-start">
                <h1 className="text-xl capitalize mt-4 ml-4 font-semibold">Shared</h1>
                <div className="w-full h-auto flex justify-between items-center">
                    ...
                    <div className="w-1/4 h-[20px] hidden lg:flex justify-center items-center">
                ...
                <section className="w-full h-auto flex flex-col justify-start items-center space-y-4 lg:space-y-0 lg:flex-row lg:justify-start lg:items-center lg:flex-wrap">
                   ...
                   {files.map((file)=>{
                      ...
                      const {type, extension} = getFileType(file.file_name);
                       return (
                       <SharedFileCard key={file.id} file = {file} type = {type} extension= {extension}/>
                      )})}
                </section>
             ):
